...
- new `loadConversation(conv: Conversation | null)` method: sets the messages and seeds `usageRecord` from `conv.usage ?? null` — the persisted record drives the meter on switch and reload without the hook touching storage. it performs no abort; callers abort separately, as they do today.
